...
sales_orders_date_from =  17  #also_ fleet_ops
sales_orders_date_to =   18 #also_ fleet_ops
...
def gsheet():
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name(r'secret_key.json', scopes=scopes)
    file = gspread.authorize(creds)

                            ############
                              #G Spread
    ########################################################################
    dry_wh = ['El-Marg', 'Basatin',  'El-Wahat', 'Mostorod','Barageel',"Saryaqus Frozen 2 pl", "Barageel 2PL"]



    wrokbook=file.open("OFD Live Tracking")   #Copy of OFD Live Tracking
...
#def salse_orders(card_id, start_day,end_day,query_name, start_month, end_month):
    ...
    df  = salse_orders(12177,ofd_date_from, ofd_date_to,"OFD Orders",7 ,7, "FROM","TO")
...
#######################################################
    ...
    #Set needed colm
    clistnew =["SALES_ORDER_ID", "WAREHOUSE","SO_STATUS", "NMV", "RUN_SHEET_ID","ACTION_DATE", "ACTION_TIME","OFD", "COURIER", "ACTION_HOUR", "PARTIAL" ,"UP_SELLING"]
...
# def salse_orders(card_id, start_day,end_day,query_name, start_month, end_month)
    ...
    df2 = salse_orders(12178,sales_orders_date_from, sales_orders_date_to,"Sales Orders", 7, 7, "FROM","TO")
################################################################
    print("----------------------------")
    print("----------------------------")
...
# def salse_orders(card_id, start_day,end_day,query_name, start_month, end_month):   
    df4 = salse_orders(16314, sales_orders_date_from, sales_orders_date_to,"Fleet OPS (Start & End)" ,  7, 7,"START","END")
################################################################
    print("----------------------------")
    print("----------------------------")
...
# ##############################################################################
    # Sales Orders Query
    # def salse_orders(card_id, start_day,end_day,query_name, start_month, end_month):
    df3 = salse_orders(12178, next_date_from, next_date_to,"Next Day Value", 7, 7, "FROM","TO")
#####################################################################
    print("-------------------------------")
    print("-------------------------------")
    ...
    supply_chain = ['Dry', 'CSDs - Dry', 'CSDs', 'CSDs - Dry - Water', 'Dry - Water', 'CSDs - Water', 'Water']
    df3= df3.fillna('')
...
def run(condition):
    ...
    while condition:
        print("Running Again... Greater Cairo\n--------------------")
